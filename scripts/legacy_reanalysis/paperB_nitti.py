...
def run_PN_N3(pn_n2_results):
    ...
    for sheet, d in pn_n2_results.items():
        a, b  = d['ceiling_a'], d['beta_b']
        N_max = d['N_max']
        N_ext = np.arange(1, int(N_max) + 1, dtype=float)
        C_func = lambda n, _a=a, _b=b: ceiling_model(n, _a, _b)
        C_ext  = C_func(N_ext)

        # ε-stopping on fit curve
        MG_ext = np.diff(C_ext) / (np.abs(C_ext[:-1]) + 1e-12)
        eps_stop = int(N_max)
        for i, g in enumerate(MG_ext):
            if g < EPSILON_THRESHOLD:
                eps_stop = int(N_ext[i + 1])
                break
        ...
        def n_star_diff(lam):
            U_vals = np.array([utility(C_func(n), n, lam, N_max)
                               for n in N_ext])
            return float(N_ext[np.argmax(U_vals)]) - eps_stop

        try:
            lo_v = n_star_diff(0.01)
            hi_v = n_star_diff(0.999)
            if lo_v * hi_v > 0:
                lam_equiv = np.nan; verdict = 'no crossing'
            else:
                lam_equiv = brentq(n_star_diff, 0.01, 0.999,
                                   xtol=1e-4, maxiter=200)
                verdict = '✓ valid' if 0.2 <= lam_equiv <= 0.99 else '⚠ edge'
                if 0.2 <= lam_equiv <= 0.99:
                    valid_bridges += 1
        except Exception:
            lam_equiv = np.nan; verdict = 'failed'

        total_bridges += 1
        results[sheet] = {
            'N_eps_extended': eps_stop,
