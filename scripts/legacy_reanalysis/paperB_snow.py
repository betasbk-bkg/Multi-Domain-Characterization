...
def run_PS_S3(ps_s2_results):
    ...
    for task, d in SNOW_DATA.items():
        ...
        a = ps_s2_results[task]['ceiling_a']
        b = ps_s2_results[task]['beta_b']
        C_func = lambda n, _a=a, _b=b: ceiling_model(n, _a, _b)

        # ε-stopping on observed data (N=2..10)
        MG = np.array([marginal_gain_discrete(C, i) for i in range(len(N))])
        eps_stop_idx = next((i for i, g in enumerate(MG)
                             if g < EPSILON_THRESHOLD), len(N) - 1)
        N_eps = float(N[eps_stop_idx])

        # ε-stopping on extended fit curve (N=1..N_max)
        N_ext = np.arange(1, int(N_max) + 1, dtype=float)
        C_ext = C_func(N_ext)
        MG_ext = np.diff(C_ext) / (C_ext[:-1] + 1e-12)
        eps_stop_ext = int(N_max)
        for i, g in enumerate(MG_ext):
            if g < EPSILON_THRESHOLD:
                eps_stop_ext = int(N_ext[i + 1])
                break

        # λ_equiv: find λ such that N*_utility = N_eps_ext
        def n_star_minus_target(lam):
            U_vals = np.array([utility(C_func(n), n, lam, N_max)
                               for n in N_ext])
            return float(N_ext[np.argmax(U_vals)]) - eps_stop_ext

        try:
            lo_v = n_star_minus_target(0.01)
            hi_v = n_star_minus_target(0.999)
            if lo_v * hi_v > 0:
                lam_equiv = np.nan
                verdict = 'no crossing'
            else:
                lam_equiv = brentq(n_star_minus_target, 0.01, 0.999,
                                   xtol=1e-4, maxiter=300)
                in_range = 0.2 <= lam_equiv <= 0.99
                verdict = '✓ valid' if in_range else '⚠ edge'
        except Exception:
            lam_equiv = np.nan
...
def main():
    ...
    # S3: Affect λ_equiv valid
    aff_leq = s3.get('Affect_avg', {}).get('lambda_equiv', None)
    checks.append(('PS_S3 Affect λ_equiv ∈ [0.2, 0.99]',
                   aff_leq is not None and 0.2 <= aff_leq <= 0.99))
