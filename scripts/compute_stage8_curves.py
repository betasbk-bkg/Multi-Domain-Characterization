...
labels_long.csv(.gz) and gold.csv first.
"""
...
from common_metrics import bootstrap_curve, summarize_curve


def resolve_csv(path: Path) -> Path:
    """Resolve canonical CSV inputs, allowing gzip compression for large files."""
    if path.exists():
        return path
    gz_path = Path(f"{path}.gz")
    if gz_path.exists():
        return gz_path
    raise FileNotFoundError(f"Missing required CSV input: {path} or {gz_path}")
...
def main():
    ...
    labels = pd.read_csv(resolve_csv(args.dataset_dir / "labels_long.csv"))
    ...
    # The grid starts at N = 1: the manuscript normalizes S(N) against C(1), so C(1) must
    # be observed rather than extrapolated from a fit to N >= 2.
    Ns = list(range(1, max_possible + 1))
    gold = None
    if args.mode == "gold_accuracy":
        ...
        gold = pd.read_csv(resolve_csv(gold_path))
